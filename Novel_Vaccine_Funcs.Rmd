...
knitr::opts_chunk$set(echo = TRUE,
  dev = c("png", "pdf"), # Render for the doc AND save as PDF
  fig.path = "NovelVaccineOutPdf/" # Optional: Saves them in a specific folder
)
```
...
## Vaccination Probabilities (Line 401)

### Functions for vaccination probability based on surrounding vaccinated individuals (V+) (Starting Line 422 and 444)
```{r}
...
neut_conf_vacc = 0.05*(SumOfVaccinated) + 0.55 #-0.0003125*(SumOfVaccinated)^2 + 0.06375*SumOfVaccinated + 0.5(12/04/24)
...
neut_hes_vacc = 0.05*(SumOfVaccinated) + 0.05 #0.003125*(SumOfVaccinated)^2 + 0.025*SumOfVaccinated + 0.1 #

```

### Vaccination by Vaccination Plots
```{r Vaccination by Surrounding Vaccinated, echo=FALSE}
...
### Functions for vaccination probability based on surrounding infected individuals (D+) (Line 418 and 422)
```{r}
#(Line 418)
...
```{r Vaccination by Infected, echo=FALSE}
...
## Attitude Transition Probabilities (Line Plot_Line 260)
...
neut_conf_transb = -0.075*(SumOfPostitives) + 0.8
#0.003125*SumOfPostitives^2 - 0.0875*SumOfPostitives + 0.8
#Confident conformist bias
pz3 = 0.99 - 0.99/(1+ exp(-13*((SumOfPostitives/8)-0.5)))

```
### Transition if Confident Plot
```{r Transition if Confident, echo=FALSE}
...
neut_hes_transb = 0.075*(SumOfPostitives) + 0.2
#0.003125*SumOfPostitives^2 + 0.0375*SumOfPostitives + 0.3
...
```{r Transition if Hesitant, echo=FALSE}
...
#### Influencer Effects (Line Plot Line 304, 362)
## If Hesitant..
```{r If Hesitant}
...
```{r Influencer Effects if Hesitant, echo =FALSE}
...
```{r, Influencer Effects if Confident, echo =FALSE}
